...
function GameRoom() {
		...
		<div className="flex flex-row bg-sky font-rubikone text-center text-white min-h-screen">
			...
			<div className="flex basis-1/2 grow justify-center min-h-screen">
				<div className="flex flex-col items-center">
					...
					<button disabled={guessForm === ''} onClick={handleGuess} className="bg-navy text-4xl py-2 px-10 mt-8 rounded-md hover:bg-green">Submit</button>
					{isBadGuess ? <label className="text-red-600">Incorrect</label> : <></>}
				</div>
			</div>
	...
	if (page === 1) {
		return (
		<div className={`flex flex-col flex-wrap grow font-rubikone text-6xl justify-center text-center text-white min-h-screen ${(remainingGuesses !== 0) ? "bg-green" : "bg-red-600"}`}>
